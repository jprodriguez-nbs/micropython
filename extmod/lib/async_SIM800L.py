...
commands = {
    ...
    'qicsgp':       {'string': 'AT+QICSGP=1,1,\"movistar.es\",\"\",\"\",0', 'timeout': 3, 'end': 'OK'},
    ...
    async def initialize(self):
        ...
        if not self.uart:
            ...
            MODEM_PWKEY_PIN_OBJ = Pin(
                self.MODEM_PWKEY_PIN, Pin.OUT) if self.MODEM_PWKEY_PIN else None
            MODEM_RST_PIN_OBJ = Pin(
                self.MODEM_RST_PIN, Pin.OUT) if self.MODEM_RST_PIN else None
            ...
            MODEM_DTR_PIN      = 32
            MODEM_RI_PIN       = 33
            MODEM_DTR_PIN_OBJ = Pin(MODEM_DTR_PIN, Pin.IN, pull=None)
            MODEM_RI_PIN_OBJ = Pin(MODEM_RI_PIN, Pin.IN, pull=None)
            ...
            MODEM_PWKEY_PIN_OBJ = machine.Pin(self.MODEM_PWKEY_PIN, machine.Pin.IN, pull=None)
            ...
            # Setup UART
            self.debug('Setup modem UART (tx={tx}, rx={rx})...'.format(rx=self.MODEM_TX_PIN, tx=self.MODEM_RX_PIN))
            ...
            # http://docs.micropython.org/en/latest/library/pyb.UART.html
                ...
                #self.uart = UART(1, baudrate=115200, timeout=250, timeout_char=250, rx=self.MODEM_TX_PIN, tx=self.MODEM_RX_PIN)
                self.uart = UART(1, baudrate=115200, rx=self.MODEM_TX_PIN, tx=self.MODEM_RX_PIN)
