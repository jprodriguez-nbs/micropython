...
class ADXL345:
    ...
    **Quickstart: Importing and using the device**
    ...
    def __init__(self, i2c, address=_ADXL345_DEFAULT_ADDRESS):
        ...
        self.logger = logging.getLogger("ADXL34x")
    ...
    def events(self):
        ...
        #has_interrupt = interrupt_source_register & (_INT_ACT | _INT_SINGLE_TAP | _INT_DOUBLE_TAP | _INT_FREE_FALL) > 0
        has_interrupt = False
        for event_type, value in self._enabled_interrupts.items():
            if event_type == "motion":
                ...
                has_interrupt = interrupt_source_register & _INT_ACT > 0 | has_interrupt
            if event_type == "tap":
                if value == 1:
                    self._event_status[event_type] = (
                        interrupt_source_register & _INT_SINGLE_TAP > 0
                    )
                    has_interrupt = interrupt_source_register & _INT_SINGLE_TAP > 0 | has_interrupt
                else:
                    self._event_status[event_type] = (
                        interrupt_source_register & _INT_DOUBLE_TAP > 0
                    )
                    has_interrupt = interrupt_source_register & _INT_DOUBLE_TAP > 0 | has_interrupt
            if event_type == "freefall":
                self._event_status[event_type] = (
                    interrupt_source_register & _INT_FREE_FALL > 0
                )
                has_interrupt = interrupt_source_register & _INT_FREE_FALL > 0 | has_interrupt
        
        self._event_status['has_int'] = has_interrupt
    ...
    def enabled_interrupts(self):
        int_enable_reg = self._read_register_unpacked(_REG_INT_ENABLE)

        r = {
            "raw": hex(int_enable_reg),
            "data_ready": (int_enable_reg & 0x80) == 0x80,
            "single_tap": (int_enable_reg & 0x40) == 0x40,
            "double_tap": (int_enable_reg & 0x20) == 0x20,
            "activity": (int_enable_reg & 0x10) == 0x10,
            "inactivity": (int_enable_reg & 0x08) == 0x08,
            "free_fall": (int_enable_reg & 0x04) == 0x04,
            "watermark": (int_enable_reg & 0x02) == 0x02,
            "overrun": (int_enable_reg & 0x01) == 0x01
        }
        return r

    @property
    def interrupt_map(self):
        int_map_reg = self._read_register_unpacked(_REG_INT_MAP)
        
        r = {
            "raw": hex(int_map_reg),
            "data_ready": "int 2" if int_map_reg & 0x80 else "int 1",
            "single_tap": "int 2" if int_map_reg & 0x40 else "int 1",
            "double_tap": "int 2" if int_map_reg & 0x20 else "int 1",
            "activity": "int 2" if int_map_reg & 0x10 else "int 1",
            "inactivity": "int 2" if int_map_reg & 0x08 else "int 1",
            "free_fall": "int 2" if  int_map_reg & 0x04 else "int 1",
            "watermark": "int 2" if int_map_reg & 0x02 else "int 1",
            "overrun": "int 2" if int_map_reg & 0x01 else "int 1"
        }
        return r
    ...
    @property
    def thresh_act(self):
        return self._read_register_unpacked_u(_REG_THRESH_ACT)

    
    @property
    def thresh_inact(self):
        return self._read_register_unpacked_u(_REG_THRESH_INACT)

    
    @property
    def thresh_ff(self):
        return self._read_register_unpacked_u(_REG_THRESH_FF)
    
    @property
    def thresh_tap(self):
        return self._read_register_unpacked_u(_REG_THRESH_TAP)
    
    @property
    def thresholds(self):
        r = {
            'act': hex(self.thresh_act),
            'inact': hex(self.thresh_inact),
            'ff': hex(self.thresh_ff),
            'tap': hex(self.thresh_tap),
        }
        return r
    ...
    def disable_freefall_detection(self):
        ...
        self._write_register_byte(_REG_INT_ENABLE, active_interrupts)
        try:
            if "freefall" in self._enabled_interrupts:
    ...
    def data_format(self):
        data_format_register = self._read_register_unpacked(_REG_DATA_FORMAT)
        r = {
            "raw": hex(data_format_register),
            "self_test": data_format_register & 0x80,
            "spi": data_format_register & 0x40,
            "int_invert": "1 = int active low" if data_format_register & 0x20 else "0 = int active high",
            "full_res": data_format_register & 0x08,
            "justify": data_format_register & 0x04,
            "range": data_format_register & 0x03,
        }
        return r
    ...
    def _read_register_unpacked_u(self, register):
        return unpack("<B", self._read_register(register, 1))[0]

    def _read_register(self, register, length):
        #self._i2c.readfrom_mem_into(self._i2c_addr, register & 0xFF, self._buffer[0:length])
        buf = self._i2c.readfrom_mem(self._i2c_addr, register & 0xFF, length)
        return buf
    ...
    def _write_register_byte(self, register, value):
        ...
        #self._i2c.writeto_mem(self._i2c_addr, register & 0xFF, self._buffer[0:1])
        self._i2c.writeto_mem(self._i2c_addr, register & 0xFF, bytearray([value]))
    ...
    def interruptChecker(self):
        ...
        try:
            ...
            a = self.acceleration
            e = self.events # Read once because we clear every time
            ff_e = e["freefall"] if "freefall" in e else False
            tap_e = e["tap"] if "tap" in e else False
            m_e = e["motion"] if "motion" in e else False
            impact_str ="Acc=[{x},{y},{z}], Dropped={ff}, Tapped={tap}, Motion={m}".format(x=a[0], y=a[1], z=a[2], ff=ff_e, tap=tap_e, m=m_e)
            #retCode = ff_e or tap_e or m_e
            retCode = tap_e or m_e
        except Exception as ex:
            self.logger.exc(ex, "Failed to check interrupts")
            
        return (retCode, impact_str)

    def sensorMotion(self):
                
        self.enable_freefall_detection(threshold=2,time=5)
        ...
        #accelerometer.enable_tap_detection(tap_count=1, threshold=2, duration = 1)
        self.enable_tap_detection(tap_count=1, threshold=30, duration = 10000)
        ...
        self.enable_motion_detection(threshold=30)
    ...
    def sensorImpactThreshold(self, th):
        ...
        if th != self._sensor_impact_th:
            ...
            self.logger.debug("ADXL34x: Change sensor impact threshold to {th}".format(th=th))
        
        self.sensorMotionThreshold(th)
        self.sensorMotion()
        self.sensorMotionThreshold(th)
