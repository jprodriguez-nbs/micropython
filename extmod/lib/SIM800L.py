...
class Modem(object):
    ...
    def initialize(self):
        ...
        while True:
            try:
                ...
                retries+=1
                if retries < 3:
                    self.debug('Error in getting modem info, retrying.. (#{})'.format(retries))
                    time.sleep(3)
                else:
                    raise
            else:
        ...
        self.debug('Ok, modem "{}" is ready and accepting commands'.format(self.modem_info))
    ...
    def execute_at_command(self, command, data=None, clean_output=True):
        ...
        self.debug('Writing AT command "{}"'.format(command_string_for_at.encode('utf-8')))
        ...
        while True:
            ...
            if not line:
                ...
                if pre_end and line_str.startswith('{}'.format(excpected_end)):
                    self.debug('Detected startwith end (and adding this line to the output too)')
                    output += line_str
                    break
    ...
    def connect(self, apn, user='', pwd=''):
        ...
        self.debug('Trying to close the bearer in case it was left open somehow..')
        try:
    ...
    def http_request(self, url, mode='GET', data=None, content_type='application/json'):
        ...
        if mode == 'GET':
            ...
            self.debug('Response status code: "{}"'.format(response_status_code))
