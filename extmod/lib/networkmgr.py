...
class NetworkMgr(object):
    ...
    def attachGSMtoPPP(cls, _uart):
        ...
        while is_connected is False:
            ...
            GPRS.active(True)
            time.sleep(1)
            print("Connect network.PPP with '{u}', '{p}'".format(u=cls._ppp_user, p=cls._ppp_password))
            GPRS.connect(authmode=GPRS.AUTH_CHAP, username=cls._ppp_user, password=cls._ppp_password)
            time.sleep(3)
    ...
    async def gprs_connect(cls):
        try:
            if cls._modem is None:
                if PINOUT.IMPORT_FROM_APP:
                    from frozen.modem import Modem
                else:
