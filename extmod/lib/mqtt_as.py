...
class MQTT_base:
    ...
    def __init__(self, config, networkMgr):
    ...
    async def _as_read(self, n, sock=None):  # OSError caught by superclass
        ...
        while size < n:
            ...
            try:
                if config['async'] is False:
                    msg_size = sock.readinto(buffer[size:], n - size)
                else:
                    if self._stream_reader is None:
                        self.dprint("_as_read Error: stream reader is None")
                    msg_size = await self._stream_reader.readinto(buffer[size:], n - size)
    ...
    def __open_connection(self, host, port):
        ...
        try:
            s.connect(ai_aux)
            self.dprint("Socket connected to {host}:{port}".format(host=host, port=port))
        except OSError as e:
            self.dprint("Failed to connect socket to {host}:{port} ({ai_aux})".format(host=host, port=port, ai_aux=ai_aux))
            if e.args[0] not in BUSY_ERRORS:
                self.dprint("mqtt_as._connect({addr}) exception {e}".format(addr=self._addr, e=str(e)))
                raise e
        
        self.dprint("Yield core._io_queue.queue_write")
        yield core._io_queue.queue_write(s)
        
        utime.sleep_ms(_DEFAULT_MS)
        ...
        self.dprint("Create uasyncio stream with the SSL socket")
        ss = asyncio.stream.Stream(self._sock)
        
        self.dprint("Return SecureSocket stream reader/writer {ss}".format(ss=str(ss)))
        gc.collect()
        self._stream_reader = ss
        self._stream_writer = ss
    ...
    async def __write(self, data):
        if config['async'] is False:
            ...
            if self._stream_writer is None:
                self.dprint("__write Error: stream writer is None")
            n = await self._stream_writer.write(data)
        return n

    async def _connect(self, clean):
        self.sock_connection_attempts = self.sock_connection_attempts + 1
        if config['async'] is False:
    ...
    async def wait_msg(self):
        try:
            if config['async'] is False:
                ...
                if self._stream_reader is None:
                    self.dprint("wait_msg Error: stream reader is None")
                res = await self._stream_reader.read(1)
        except OSError as e:
...
class MQTTClient(MQTT_base):
    def __init__(self, config, networkMgr):
        super().__init__(config, networkMgr)
    ...
    async def connect(self, *, quick=False):  # Quick initial connect option for battery apps
        ...
        try:
            ...
            self.dprint("Exception when connecting: {e}".format(e=str(ex)))
            self._close()
        ...
        self.dprint("mqtt_as.connect finished.")
