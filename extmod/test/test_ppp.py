...
def test_ppp():
    ...
    ppp.connect(authmode=ppp.AUTH_PAP, username=PPP_USER, password=PPP_PSW)
...
class InitSimData():
    ...
    def rquestIPwithAPN(self):
        self.gsm.write('AT+CGDCONT=1,"IP","'+APN+'",\r\n')
        status = self.read_status()
    ...
    def attachGSMtoPPP(self):
        ...
        GPRS.active(True)
        GPRS.connect(authmode=GPRS.AUTH_PAP, username=PPP_USER, password=PPP_PSW)
        if GPRS.isconnected():
            print(GPRS.ifconfig())
            return True
        else:
